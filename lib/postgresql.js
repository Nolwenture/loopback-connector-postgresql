...
PostgreSQL.prototype._modifyOrCreate = function (
  model,
  data,
  options,
  fields,
    ...
    if (!fields.properties[i].id) {
      setValues.push(
        new ParameterizedSQL(
          fields.names[i] + '=' + columnValues[i].sql,
...
PostgreSQL.prototype.save = PostgreSQL.prototype.updateOrCreate = function (
  model,
  data,
  options,
  cb
...
PostgreSQL.prototype.getInsertedId = function (model, info) {
  var insertedId =
    info && typeof info.insertId === 'number' ? info.insertId : undefined;
  return insertedId;
};
...
PostgreSQL.prototype.updateOrCreate = function (model, data, callback) {
  ...
  query.push(
    idColumns
      .map(function (key, i) {
        return (
          (i > 0 ? " AND " : " ") + key + "=$" + (nonIdsInData.length + i + 1)
        );
      })
      .join(",")
  );
  ...
  query.push(
    ", insert_outcome AS (INSERT INTO ",
    self.tableEscaped(model),
    " "
  );
  query.push(self.toFields(model, data, true));
  query.push(
    " WHERE NOT EXISTS (SELECT * FROM update_outcome) RETURNING ",
    idColumns.join(","),
    ")"
  );
  query.push(
  ...
  if (filter) {
    ...
    if (filter.where) {
      ...
      if (joins.joinStmt.sql) {
        ...
        selectStmt.sql = selectStmt.sql.replace(
          'SELECT',
          'SELECT DISTINCT ' + tableAlias + `.${idNames[0]}, `
        );
  ...
  for (var key in where) {
    ...
    if (key === 'and' || key === 'or') {
      ...
      if (Array.isArray(clauses)) {
        for (var i = 0, n = clauses.length; i < n; i++) {
          var stmtForClause = self.buildJoins(
            model,
            clauses[i],
            tableAlias,
            aliases
    ...
    if (p == null) {
      ...
      if (relations && relations[key]) {
        ...
        var recursiveResult = self.buildJoins(
          relation.model,
          where[key],
          childTableAlias
        );
...
PostgreSQL.prototype._buildWhere = function (
  model,
  where,
  tableAlias,
  aliases
  ...
  for (var key in where) {
    ...
    if (key === 'and' || key === 'or') {
      ...
      if (Array.isArray(clauses) && clauses.length > 0) {
        for (var i = 0, n = clauses.length; i < n; i++) {
          var stmtForClause = self._buildWhere(
            model,
            clauses[i],
            tableAlias,
            aliases
    ...
    if (p == null) {
      ...
      if (relations && relations[key]) {
        let relation = relations[key];
        let childWhere = self._buildWhere(
          relation.model,
          where[key],
          aliases[relation.model],
          aliases
        );
    ...
    var columnName = tableAlias
      ? tableAlias + '.' + columnEscaped
      : columnEscaped;
...
PostgreSQL.prototype.count = function (model, where, options, cb) {
  ...
  var aliases = {};
  if (joins.joinStmt.sql) {
    stmt.sql = stmt.sql.replace(
      'count(*)',
      'COUNT(DISTINCT ' + tableAlias + '.id)'
    );
  ...
  this.execute(stmt.sql, stmt.params, function (err, res) {
    if (err) {
      return cb(err);
    }
